  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message:
          "Please provide partnerId/clientId, type, transactionType, amount, and currency",
      });
    }
...
exports.getReports = async (req, res) => {
  try {
    ...
    if (id) {
      filter.$or = [{ partnerId: id }, { clientId: id }];
    }
    if (type) {
      filter.type = type;
    }
...
exports.updateReport = async (req, res) => {
  try {
    ...
    if (!type || !transactionType || !amount || !currency) {
      return res.status(400).json({
        success: false,
        message: "Please provide type, transactionType, amount, and currency",
      });
    }
